...
extern "C" oe_result_t
oe_call_enclave_function(oe_enclave_t *enclave, uint32_t function_id,
...

  {
    ...
    enclave->_ecall_table[function_id](
        static_cast<const uint8_t *>(enc_input_buffer), input_buffer_size,
        static_cast<uint8_t *>(enc_output_buffer), output_buffer_size,
...
oe_result_t oe_create_enclave(const char *path, oe_enclave_type_t type,
                              uint32_t flags,
                              const oe_enclave_setting_t *settings,
                              uint32_t setting_count,
                              const oe_ocall_func_t *ocall_table,
                              uint32_t num_ocalls, oe_enclave_t **enclave) {
  OE_UNUSED(path);
  OE_UNUSED(type);
  OE_UNUSED(flags);
  OE_UNUSED(settings);
...
uint32_t oe_get_create_flags(void) { return 0; }
}
