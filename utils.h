...
template <typename A, typename C> bool in(const A &a, const C &c) {
  return std::find(c.begin(), c.end(), a) != c.end();
}

template <typename C1, typename C2> void append(C1 &c1, const C2 &c2) {
  c1.insert(c1.end(), c2.begin(), c2.end());
}

template <typename T> void autogen_preamble(T &os) {
  os << "/*"
     << " *  This file is auto generated by oeedger8r. DO NOT EDIT."
     << " */";
}

template <typename T> void header(T &os, const std::string &guard) {
  autogen_preamble(os);
  // clang-format off
    os << "#ifndef " + guard
       << "#define " + guard;
...
template <typename T> void footer(T &os, const std::string &guard) {
  os << "#endif // " + guard;
}
...
inline std::string replace(std::string s, const std::string &p,
                           const std::string &q) {
  size_t pos = s.find(p);
...
inline std::string mdecl_str(const std::string &name, Type *t, Dims *dims,
                             Attrs *attrs) {
...
inline std::string prototype(const Function *f, bool ecall = true,
                             bool gen_t = true) {
  std::string retstr = (ecall != gen_t) ? "oe_result_t" : atype_str(f->rtype_);
...
inline std::string psize(Decl *p, const std::string &prefix = "") {
  ...
  if (p->attrs_ && !p->attrs_->count_.is_empty())
    return "((size_t)" + count_attr_str(p->attrs_->count_, prefix) + " * " + s +
           ")";
...
inline UserType *get_user_type(const std::vector<UserType *> &types,
